...
├── script.js                          四页切换、会议章程、章节导航与 REWIRE 交互
├── README.md                          运行与修改说明
└── assets/
...
网站默认打开独立会议章程首页，顶部包含四个一级栏目：

1. `00｜会议章程`：三个会议环节的横向顺序目录，点击整张卡片进入对应页面。
2. `01｜新一代技术蓝图构建方式`：完整的方法论、目标体系与技术成果。
3. `02｜后续技术蓝图设计工作计划`：四阶段推进计划和五项最终成果。
4. `03｜现状调研框架与重点问题`：七大调研维度和十一项重点调研问题。

会议章程首页包括：

- `01｜新一代技术蓝图构建方式`：从北燃未来业务目标出发，介绍面向数智时代的技术能力与系统生态构建方式。
- `02｜后续技术蓝图设计工作计划`：后续调研、方案设计、研讨到成果汇报的整体工作计划与时间安排。
- `03｜现状调研框架与重点问题`：本轮技术调研的整体框架、重点领域及需要重点了解的核心问题。
- 三张大字号议程卡片在桌面端横向排列，在移动端自动调整为纵向排列。
...
- 修改默认会议章程首页及三个议题：编辑 `index.html` 中 `id="page-agenda"` 的内容。
